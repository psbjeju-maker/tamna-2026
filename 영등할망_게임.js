...
function gSail(){
  ...
  var rush={state:'idle',until:0,nextAt:26,spawnTimer:0};
  var boss={alive:false,hp:0,maxHp:5,x:0,y:0,r:92,v:1.15,asset:null,nextAt:34,flashUntil:0};
  ...
  function loop(now){
    ...
    /* 골든 러시 — 5초간 복이 잔뜩 쏟아지고 위험은 전혀 없다 */
    if(rush.state==='idle' && el>=rush.nextAt && !pick.open && !boss.alive){
    ...
    /* 미니보스 — 레벨이 오를수록 점점 더 자주 나온다("보스도 개많이") */
    if(!boss.alive && el>=boss.nextAt && !pick.open && rush.state!=='active'){
      boss.alive=true; boss.hp=5; boss.maxHp=5;
    ...
    var suspendSpawn = pick.open || rush.state==='active';

    if(!suspendSpawn && !inFinale && !ending){
      var spawnRate = tutorialDone ? 1 : 0.45;
      spawn-=sdt;
      if(spawn<=0){
        spawn=Math.max(110,(620-el*22-lv*4)/spawnRate);
        var rk=Math.floor(Math.random()*ASSETS.rockPool.length);
        var zz = lv>=8 && Math.random()<0.35;
        rocks.push({x:60+Math.random()*(W-120),y:-60,r:32+Math.random()*26,v:3.6+el*0.11+lv*0.15,asset:ASSETS.rockPool[rk],passed:false,dead:false,justOk:true,zigzag:zz,zzPhase:Math.random()*Math.PI*2});
      }
      foodSpawn-=sdt;
      if(foodSpawn<=0){
    ...
    if(boss.alive){
      ...
      for(var bmi=missiles.length-1;bmi>=0;bmi--){
        ...
        if(Math.hypot(bm.x-boss.x,bm.y-boss.y)<boss.r+8){
          ...
          if(boss.hp<=0){
            ...
            boss.alive=false; boss.nextAt=el+Math.max(20,45-lv*0.6);
            break;
          }
        }
      }
      if(boss.alive){
        ...
        if(boss.y>H+160){ boss.alive=false; boss.nextAt=el+Math.max(20,45-lv*0.6); }
      }
    }
