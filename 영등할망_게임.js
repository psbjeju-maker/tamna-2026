...
function gSail(){
  ...
  var GUST_LINES=['이짝저짝 요망지게 댕기멍 보름 탐져이!','이번엔 좀 세게 불어볼거여!'];
  var HIT_LINES=['아이고게, 정신 촐리라게!','아이고 놀란 것 좀 보라!'];
  var END_LINES=['제라하게 보름 탈 줄 알암쪄이','오늘은 여기까지 하게마씨'];

  /* ---- 업그레이드 — 뱀서라이크처럼 시간마다 3장 중 하나를 직접 탭해서 고른다 ----
     기존 "1초 안에 배를 조종해 통로로 들어가기"는 뭘 고르는 건지 알기 어렵다는
     피드백을 받아, 카드 3장을 눈으로 보고 직접 눌러 고르는 방식으로 바꿨다. */
  var upg={fire:0,spread:0,heart:0,just:0,bok:0,speed:0};
  var UPGRADES={
    fire:  {emoji:'💥', title:'포격 강화', desc:'연사 속도가 빨라진다'},
    spread:{emoji:'🎯', title:'다연장 확장', desc:'포탄이 하나 더 나간다'},
    heart: {emoji:'❤️', title:'바람의 가호', desc:'최대 체력이 늘고 즉시 회복'},
    just:  {emoji:'✨', title:'회피의 감', desc:'회피(JUST) 판정이 후해진다'},
  ...
  function applyUpgrade(key){
    upg[key]++;
    if(key==='heart'){ maxHearts++; hearts=Math.min(maxHearts,hearts+1); }
    toast(UPGRADES[key].emoji+' '+UPGRADES[key].title+'!');
    sfx('bonus',0.6);
  }
  function shuffle(a){ for(var i=a.length-1;i>0;i--){var j=Math.floor(Math.random()*(i+1));var t=a[i];a[i]=a[j];a[j]=t;} return a; }

  var pick={open:false,options:[],until:0};
  var pickRound=0, nextPickAt=10;
  var PICK_TITLES=['바람이 하나를 골라달라 한다','요망진 걸로 골라보라이','무사 하나만 고르라!'];
  var pickTitle='';
  function openPick(el){
    pick.options=shuffle(UPG_KEYS.slice()).slice(0,3);
    pick.open=true; pick.until=el+8;
    pickTitle=pickRandom(PICK_TITLES);
    sfx('evolve',0.5);
  }
  function closePick(key,el){
    pick.open=false;
    applyUpgrade(key);
    pickRound++;
    nextPickAt=el+Math.min(26,14+pickRound*2);
  }
  ...
  var gustEvt={state:'idle',until:0,dir:1,nextAt:18};
  var finaleUntil=0,inFinale=false,finaleKills=0;
  var awakened=false;
  var ending=false,endingT0=0,softCapDone=false;
  ...
  /* 진화 게이지 증가 + 단계 상승 처리 — 먹거리/JUST/돌풍 등 여러 곳에서 호출 */
  ...
  function loop(now){
    ...
    /* ---- 조작 입력 ---- (선택 카드가 떠 있을 땐 배가 움직이지 않는다) */
    if(!pick.open){
    ...
    /* 업그레이드 선택 카드 — 정해진 간격마다 반복해서 뜬다(뱀서류처럼 계속 성장) */
    if(!pick.open && el>=nextPickAt && !inFinale && !ending && gustEvt.state==='idle'){
      openPick(el);
    }
    ...
    /* 돌풍 이벤트 — 한 번만이 아니라 계속 반복해서 찾아온다 */
    if(gustEvt.state==='idle' && el>=gustEvt.nextAt && !pick.open){
      gustEvt.state='cutin';
      showCutin(pickRandom(GUST_LINES),1700);
    ...
    if(gustEvt.state==='blow' && el>=gustEvt.until){
      ...
      gustEvt.nextAt=el+22+Math.random()*8;
      gustEvt.state='idle';
    }
    ...
    /* 죽지 않으면 계속 오래 할 수 있게 — 시간으로 강제 종료하지 않는다.
       다만 행사 특성상 한 사람이 무한정 붙잡고 있을 수는 없어 3분 안전장치만 둔다. */
    if(!softCapDone && el>=180){ softCapDone=true; triggerEnding(el); }
    if(ending && el-endingT0>3.6 && alive){
      alive=false; finishSail(el,kills,justCount); return;
    }

    var suspendSpawn = (gustEvt.state==='cutin') || pick.open;
    var mul = (gustEvt.state==='blow') ? 1.12 : 1;
    ...
    var speedFeel=1+ (inFinale?0.9:0) + (gustEvt.state==='blow'?0.2:0);
    ...
    if(pick.open){
      ...
      pick.options.forEach(function(key,i){
        ...
        ctx.fillText(UPGRADES[key].emoji, cx, cy0+58);
        ctx.font='800 17px SCDream, sans-serif';ctx.fillStyle='#F2F0EA';
        ctx.fillText(UPGRADES[key].title, cx, cy0+92);
        ...
        ctx.fillText(dtxt, cx, cy0+116);
        ctx.fillStyle='rgba(255,255,255,.14)';ctx.fillRect(cx0+12,cy0+cardH-14,cardW-24,5);
        ctx.fillStyle='#F5B331';ctx.fillRect(cx0+12,cy0+cardH-14,(cardW-24)*pctLeft,5);
      });
    }
    ...
    if(hit && !invuln && !inFinale && !ending && !pick.open){
      hearts--;
