...
var CATCH_KIND={
  small: {r:24, need:1, val:8,   vMul:1.25, img:'orange_small',  c1:'#FFB347',c2:'#E0741B'},
  normal:{r:34, need:1, val:12,  vMul:1,    img:'orange_normal', c1:'#FFA83A',c2:'#D96A14'},
  big:   {r:44, need:2, val:26,  vMul:0.8,  img:'orange_big',    c1:'#FFA83A',c2:'#D96A14'},
  king:  {r:56, need:3, val:46,  vMul:0.6,  img:'orange_king',   c1:'#FF9F2E',c2:'#CF5F10'},
  gold:  {r:38, need:1, val:80,  vMul:0.85, img:'orange_gold',   c1:'#FFE27A',c2:'#E5A11C', gold:true},
  legend:{r:66, need:3, val:100, vMul:0.42, img:'orange_legend', c1:'#FFF0A8',c2:'#E5A11C', gold:true, legend:true},
  rotten:{r:36, need:1, val:-30, vMul:0.95, img:'orange_rotten', c1:'#6B4A2B',c2:'#2E2418', rotten:true}
};
...
function gCatch(){
  ...
  function spawnFruit(kind,x){
    ...
    var speed=(2.0+Math.random()*1.2+elapsed/22000)*k.vMul*(fever?1.12:1);
    items.push({kind:kind,k:k,x:x!=null?x:70+Math.random()*(W-140),y:-k.r-20,r:k.r,need:k.need,hits:0,val:k.val,
  ...
  function doSpawn(){
    ...
    /* 전설의 황금귤: 15초 이후 판마다 최대 1회, 매 스폰 5% */
    if(!legendSpawned&&elapsed>15000&&Math.random()<0.05){spawnFruit('legend',W*0.3+Math.random()*W*0.4);return}
    var base=60+Math.random()*(W-120);
    for(var i=0;i<n;i++){
      kind=pickCatchKind(p);
      /* 여러 개 동시 등장은 한 번에 그어 다 자를 수 있게 옆으로 나란히 */
      var x=n===1?base:Math.max(60,Math.min(W-60,base+(i-(n-1)/2)*95));
      spawnFruit(kind,x);
    }
  }
  ...
  function juice(x,y,ang,kind,n){
    ...
    for(var i=0;i<n;i++){
      ...
      fx.push({x:x,y:y,vx:Math.cos(a)*sp,vy:Math.sin(a)*sp-1,l:1,c:cols[i%3],sz:2.5+Math.random()*3.5,dec:0.035+Math.random()*0.02});
    }
    var jn=kind==='rotten'?'juice_rotten':'juice'+(1+Math.floor(Math.random()*3));
    sprites.push({im:jn,x:x,y:y,rot:ang,w:CATCH_KIND[kind].r*2.4,w1:CATCH_KIND[kind].r*4.2,life:1,dec:0.07,add:kind!=='rotten'});
  ...
  function slice(p0,p1){
    ...
    for(var i=items.length-1;i>=0;i--){var it=items[i];
      ...
      if(segPointDist(p0,p1,it.x,it.y)<it.r+20){
        ...
        if(it.k.rotten){
          ...
          sfx('boom',0.45);popText(it.x,it.y-it.r-10,'썩은 귤! -30','#7A4C2A',24);
  ...
  function drawPiece(p){
    ...
    ctx.rotate(a);ctx.translate(0,r*0.12);ctx.scale(1,0.55);
  ...
  function drawHUD(left){
    ...
    ctx.save();
    ctx.fillStyle='rgba(0,0,0,.18)';ctx.fillRect(0,0,W,8);
    ctx.fillStyle=barC;ctx.fillRect(0,0,W*pct,8);
    var secs=Math.ceil(left/1000),hudC=fever?'#FFD84A':'#FFFFFF';
    ctx.textBaseline='alphabetic';
    ctx.textAlign='left';ctx.font='800 15px SCDream, sans-serif';ctx.fillStyle='rgba(255,255,255,.75)';ctx.fillText('TIME',24,40);
    ctx.font='800 40px SCDream, sans-serif';ctx.lineJoin='round';ctx.strokeStyle='rgba(60,30,0,.55)';ctx.lineWidth=6;ctx.strokeText(String(secs),24,80);
    ctx.fillStyle=secs<=5?'#F0605A':hudC;ctx.fillText(String(secs),24,80);
    ctx.textAlign='right';ctx.font='800 15px SCDream, sans-serif';ctx.fillStyle='rgba(255,255,255,.75)';ctx.fillText('SCORE',W-24,40);
    ctx.font='800 40px SCDream, sans-serif';var s=String(score);while(s.length<5)s='0'+s;
    ctx.strokeText(s,W-24,80);ctx.fillStyle=fever?'#FFD84A':'#F5B331';ctx.fillText(s,W-24,80);
    if(combo>=2&&elapsed<comboUntil+300){
      ...
      ctx.save();ctx.translate(W/2,110);ctx.scale(pop,pop);ctx.globalAlpha=Math.min(1,k*2);
  ...
  function drawBanners(){
    banners.forEach(function(b){
      ...
      var sc=(b.type==='ildo'?1.6:1.15)-ain*(b.type==='ildo'?0.5:0.15),x=b.x||W/2,y=b.y||H*0.36;
  ...
  function drawTrail(){
    ...
    for(var pass=0;pass<2;pass++){
      for(var i=1;i<trail.length;i++){
        ...
        if(pass===0){ctx.strokeStyle='rgba(79,227,190,'+(al*0.38).toFixed(2)+')';ctx.lineWidth=Math.max(3,26*al)}
        else{ctx.strokeStyle='rgba(255,255,255,'+(al*0.95).toFixed(2)+')';ctx.lineWidth=Math.max(1.2,9*al)}
  ...
  function drawResult(){
    ...
    var cw=Math.min(W-80,620),ch=Math.min(H-160,820),cx=W/2,cy=H/2+(1-ease)*60;
    ctx.globalAlpha=ease;ctx.translate(cx,cy);
    ctx.fillStyle='#FFF8EC';ctx.strokeStyle='#E5A11C';ctx.lineWidth=6;
    rrect(-cw/2,-ch/2,cw,ch,28);ctx.fill();ctx.stroke();
    var chest=cimg('chest');if(chest)drawImgC(chest,0,-ch/2+10,cw*0.42);
    var y=-ch/2+cw*0.2+40;
    drawText('황금귤 수확 완료!',0,y,38,'#8A4B12');y+=64;
    var isBest=score>(S.best.catch||0);
    var rows=[['수확한 귤',cnt.fruit+'개'],['황금귤',cnt.gold+'개'],['최고 콤보',bestCombo+' COMBO'],['썩은 귤',cnt.rotten+'개']];
    ctx.textBaseline='middle';
    rows.forEach(function(r){
      ctx.textAlign='left';ctx.font='600 24px SCDream, sans-serif';ctx.fillStyle='#8A6A4A';ctx.fillText(r[0],-cw/2+44,y);
      ctx.textAlign='right';ctx.font='800 28px SCDream, sans-serif';ctx.fillStyle='#3A2A18';ctx.fillText(r[1],cw/2-44,y);
      ctx.fillStyle='rgba(138,106,74,.18)';ctx.fillRect(-cw/2+44,y+24,cw-88,2);y+=56;
    });
    y+=18;drawText('최종 점수',0,y,20,'#8A6A4A',null,600);y+=58;
    drawText(score.toLocaleString(),0,y,66,'#E5A11C','rgba(120,60,0,.35)');y+=56;
    drawText(catchRank(score),0,y,24,'#3A2A18',null,600);y+=48;
    if(isBest){ctx.save();ctx.translate(0,y);ctx.scale(1+Math.sin(resultT/120)*0.04,1+Math.sin(resultT/120)*0.04);drawText('NEW RECORD!',0,0,32,'#F0605A','rgba(255,255,255,.9)');ctx.restore();y+=44}
    if(resultReady){ctx.globalAlpha=ease*(0.6+Math.sin(resultT/200)*0.4);drawText('화면을 눌러 복 받기',0,ch/2-40,20,'#8A6A4A',null,600)}
    ctx.restore();
  }
  ...
  function loop(now){
    ...
    if(!finished){
      ...
      if(secs<=5&&secs>=1&&secs!==countdownShown&&left>0){countdownShown=secs;banners.push({txt:String(secs),type:'cd',t:0,y:H*0.42});if(secs<=3)beep(secs===1?1100:800,0.12,'square',0.12)}
    ...
    clouds.forEach(function(c){var im=cimg(c.im);if(im)drawImgC(im,c.x,c.y,c.w,0,0.35)});
    ctx.save();
